    ...
    <div className="flex-1 bg-[#F5F6FA] dark:bg-slate-950 overflow-y-auto">
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">Live message queue and worker status</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Pending', value: stats?.pendingMessages, color: 'amber' },
            { label: 'Active Workers', value: stats?.activeWorkers, color: 'emerald' },
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                {messages.map((m) => (
                  <tr key={m._id}>
                    ...
                    <td className="py-3 font-mono text-xs">{m.workerId || '—'}</td>
        ...
        <p className="text-xs text-slate-400 text-center">
          ...
          — configure Redis and webhook URLs for reliable delivery
        </p>
      </div>
    </div>
  );
