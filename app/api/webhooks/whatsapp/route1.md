...
export async function POST(req: NextRequest) {
  try {
    ...
    // Evolution API V2 sends uppercase with underscores (e.g., MESSAGES_UPSERT)
    // V1 sends lowercase with dots (e.g., messages.upsert)
    ...
    if (eventName === 'connection.update') {
      ...
      if (state === 'open') {
        console.log(`[WHATSAPP WEBHOOK] Connection OPEN for ${instanceName}`);
        await Profile.updateOne(
          { $or: [{ workerId: instanceName }, { instanceName }] },
          { status: 'active', lastPing: new Date() }
        );
      } else if (state === 'close') {
        await Profile.updateOne(
          { $or: [{ workerId: instanceName }, { instanceName }] },
    ...
    if (eventName === 'messages.upsert') {
      ...
      // Find the locationId by checking Profile (workerId or instanceName)
      ...
      if (locationId && !fromMe) {
        try {
          ...
          ghlError = err.response?.data ? JSON.stringify(err.response.data) : err.message;
          console.error('Failed to inject inbound WhatsApp message to GHL:', ghlError);
        }
      }
      ...
      return NextResponse.json({ saved: true, ghlInjected, ghlError });
    }
    return NextResponse.json({ success: true });
  } catch (error) {
